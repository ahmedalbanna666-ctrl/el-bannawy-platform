...
export default function PrivateMonthlyPage(): ReactNode {
  ...
  const selectedSchedule = useMemo(
    () => privateSchedules.find((s) => s.id === scheduleId) ?? null,
    [privateSchedules, scheduleId],
  );
  ...
  const nextSessionDate = useMemo(() => {
    ...
    for (let i = 0; i < 14; i += 1) {
      ...
      const dow = DAY_VALUES[d.getDay()] ?? d.getDay();
      if (sortedDays.some((day) => day.dayOfWeek === dow)) {
    ...
    <div className="flex flex-col gap-6 pb-4">
      ...
      {step === 1 && (
        <div className="flex flex-col gap-5">
          ...
          ) : privateSchedules.length === 0 ? (
            <LiveEmpty
              icon={<User className="h-10 w-10" />}
              title="لا توجد جداول متاحة حالياً"
              description="لم يفتح المعلمون مواعيد خاصة بعد."
            />
          ) : (
            <div className="grid gap-3 md:grid-cols-2">
              {privateSchedules.map((s) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      selected
                        ? "border-primary-400/60 ring-2 ring-primary-400/50"
                        : "border-neutral-200/70 dark:border-white/10",
                    )}
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary-500/15 text-primary-500">
