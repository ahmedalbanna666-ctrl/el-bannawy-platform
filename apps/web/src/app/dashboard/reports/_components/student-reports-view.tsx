...
export function StudentReportsView(): ReactNode {
  const { data: report, isLoading, isError, error } = useQuery<StudentReport | null>({
    queryKey: ["reports-my"],
    queryFn: async () => {
  ...
  if (isError) return <ErrorState title="فشل تحميل التقارير" description={error instanceof Error ? error.message : "حدث خطأ"} />;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div>
        ...
        <p className="mt-0.5 text-sm text-neutral-500">تقدمك التعليمي وتحليلات أدائك</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard icon={ClipboardList} label="نتيجة الواجبات" value={`${String(report.homework.avgScore)}%`} sub={`نسبة نجاح ${String(report.homework.passRate)}%`} variant="warning" />
