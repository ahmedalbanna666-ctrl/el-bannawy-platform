import type { QuestionData, QuestionAnswer, QuestionValidationResult, QuestionOptionData } from "./types";

interface ValidatableOption extends QuestionOptionData {
  readonly isCorrect: boolean;
}

interface ValidatableQuestion extends Omit<QuestionData, "options"> {
...
export function validateQuestion(
  ...
  switch (question.type) {
    ...
    case "FILL_BLANK":
      return validateFillBlank(question, answer);
    case "MATCHING":
      return validateMatching(question, answer);
    case "ORDERING":
      return validateOrdering(question, answer);
    default:
      return {
        isValid: false,
        correct: false,
        score: 0,
        maxScore: 1,
        errors: [`Unknown question type: ${(question as { type: string }).type}`],
      };
  }
}
...
function validateFillBlank(
  ...
  const correct = correctOptions.some(
    (opt) => opt.text.toLowerCase() === userAnswer,
  );
...
function validateMatching(
  ...
  return {
    ...
    score: 0,
    maxScore: 1,
    errors: ["Matching validation not yet implemented on frontend"],
  };
}
...
function validateOrdering(
  ...
  const matches = answer.selectedOptionIds.every(
    (id, idx) => idx < correctOrder.length && id === correctOrder[idx],
  );
