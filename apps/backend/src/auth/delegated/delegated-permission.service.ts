...
const CEILINGS: Record<UserRole, readonly Permission[]> = {
  ...
  TEACHER: [
    ...
    PERMISSIONS.LIVE_VIEW, PERMISSIONS.LIVE_CREATE, PERMISSIONS.LIVE_EDIT, PERMISSIONS.LIVE_DELETE, PERMISSIONS.LIVE_CONTROL,
    PERMISSIONS.MISTAKES_VIEW, PERMISSIONS.MISTAKES_PRACTICE,
  ...
  SUPPORT: [
    PERMISSIONS.SUPPORT_ANSWER,
    PERMISSIONS.LESSONS_VIEW, PERMISSIONS.UNITS_VIEW,
    PERMISSIONS.LIVE_VIEW, PERMISSIONS.REPORTS_VIEW,
    PERMISSIONS.NOTIFICATIONS_SEND,
    PERMISSIONS.LEARNING_ACCESS, PERMISSIONS.COINS_VIEW,
  ],
  ...
  STUDENT: [
    ...
    PERMISSIONS.AI_MANAGE,
    PERMISSIONS.COINS_VIEW, PERMISSIONS.COINS_PURCHASE, PERMISSIONS.COINS_UNLOCK,
    PERMISSIONS.MISTAKES_VIEW,
    PERMISSIONS.MISTAKES_PRACTICE,
    PERMISSIONS.SAVED_PDFS_VIEW, PERMISSIONS.SAVED_PDFS_DELETE, PERMISSIONS.SAVED_PDFS_DOWNLOAD,
  ],
};
...
export class DelegatedPermissionService {
  ...
  async getEffectivePermissions(userId: string): Promise<readonly Permission[]> {
    ...
    if (user.role === "SUPPORT" || user.role === "SECRETARY") {
      return [...new Set(grants.map((g) => g.permission as Permission))].filter(
        (p) => ceiling.includes(p),
      );
    }
